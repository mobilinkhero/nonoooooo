...
async function handleMessageChange(value: any) {
  ...
  // LOG INCOMING WEBHOOK START
  await storage.logApiRequest({
    channelId: channel.id,
    requestType: 'webhook_messages_received',
    endpoint: 'handleMessageChange',
    method: 'POST',
    requestBody: { messagesCount: messages.length, from: messages[0]?.from },
    responseStatus: 200,
  ...
  for (const message of messages) {
    ...
    let contact = await storage.getContactByPhoneAndChannel(normalizedFrom, channel.id);
    let isNewConversation = false;

    console.log(`[Webhook] Incoming from: ${from}, normalized: ${normalizedFrom}, channel: ${channel.id}`);

    if (!contact) {
      console.log(`[Webhook] Contact not found, creating...`);
      ...
      contact = { ...contact, name: normalizedFrom };
    }

    // Find/create conversation (channel-scoped)
    let conversation = await storage.getConversationByPhoneAndChannel(normalizedFrom, channel.id);
    console.log(`[Webhook] Conversation lookup result: ${conversation ? 'found (ID: ' + conversation.id + ')' : 'not found'}`);

    if (!conversation) {
      isNewConversation = true;
      console.log(`[Webhook] Marking as new conversation`);
    ...
    try {
      ...
      console.log(`[Webhook] Processing message for conv=${conversation.id}, isNewConversation=${isNewConversation}`);

      if (isNewConversation) {
        console.log(`[Webhook] Triggering handleNewConversation for ${conversation.id}`);
...
async function checkAndSendAiReply(
  ...
  let hasMatch = triggerWords.length === 0;
  if (!hasMatch && Array.isArray(triggerWords) && triggerWords.length > 0) {
  ...
  if (!hasMatch) {
    ...
    if (!hasAiHistory) {
      console.log(`[AI] Skipping reply: No trigger match and no AI history. Words defined: ${triggerWords.join(', ')}`);
      return false;
    }

    console.log(`[AI] Continuing conversation (existing history) for message: "${messageContent}"`);
  } else {
    console.log(`[AI] Processing reply: ${triggerWords.length === 0 ? 'Global trigger (no words)' : 'Trigger word matched'} for message: "${messageContent}"`);
  }
  ...
  // Mark incoming user message as read right away because AI will reply 
  try {
    const latestIncoming = conversationHistory.find((m) => m.fromUser && m.whatsappMessageId);
    if (latestIncoming) {
      await whatsappApi.markMessageAsRead(latestIncoming.whatsappMessageId);
    }
  } catch (e) {
    console.error("[AI] Error marking message read:", e);
  }

  // Notify inbox UI that AI is "typing"
  try {
    const { io } = await import("../socket");
    if (io) {
      io.to(`conversation:${conversation.id}`).emit('agent_typing', {
        conversationId: conversation.id,
  ...
  try {
    const { io } = await import("../socket");
    if (io) {
      io.to(`conversation:${conversation.id}`).emit('agent_stopped_typing', {
        conversationId: conversation.id
      });
    }
  } catch (e) {
  ...
  try {
    console.log(`[AI] Sending reply to ${conversation.contactPhone}: "${aiResponse.substring(0, 50)}..."`);
    const result = await whatsappApi.sendTextMessage(
      conversation.contactPhone,
      aiResponse
    );

    // AI-Reply Sent Success Log
    await storage.logApiRequest({
      channelId: conversation.channelId,
      requestType: 'ai_auto_reply',
      endpoint: `${whatsappApi.baseUrl}/${whatsappApi.channel.phoneNumberId}/messages`,
      method: 'POST',
      requestBody: { to: conversation.contactPhone, content: aiResponse },
      responseStatus: 200,
      responseBody: result,
      duration: 0
    });

    // Save AI response as outbound message in DB
    ...
    // Broadcast AI message via WebSocket for real-time UI updates
    ...
    // Log error to DB for developer visibility
    await storage.logApiRequest({
      channelId: conversation.channelId,
      requestType: 'ai_reply_error',
      endpoint: 'sendTextMessage',
      method: 'POST',
      requestBody: { to: conversation.contactPhone, error: error.message },
      responseStatus: error.response?.status || 500,
      responseBody: error.response?.data || error.stack,
      duration: 0
    });
...
async function generateAiResponse(
  ...
  try {
    ...
    // 1. Find Site associated with this channel for RAG
    const [site] = await db.select().from(sites).where(eq(sites.channelId, channelId)).limit(1);
    let trainingContext = "";

    if (site) {
      console.log(`[AI] Searching training data for site: ${site.name} (${site.id})`);
      const trainingData = await searchTrainingData(site.id, channelId, userMessage);

      if (trainingData.chunks.length > 0 || trainingData.qaPairs.length > 0) {
        trainingContext = "\n\nUse the following training data context to answer the user's question. If the answer is not in the context, respond based on your general knowledge but mention that this is general information if relevant.\n\n--- TRAINING DATA ---\n";
        trainingContext += trainingData.chunks.join("\n\n");
        if (trainingData.qaPairs.length > 0) {
          trainingContext += "\n\n--- FAQ PAIRS ---\n";
          trainingData.qaPairs.forEach(qa => trainingContext += `Q: ${qa.question}\nA: ${qa.answer}\n\n`);
        }
        console.log(`[AI] Found ${trainingData.chunks.length} chunks and ${trainingData.qaPairs.length} QA pairs for context`);
      }
    }
    ...
    const messages: any[] = [
      {
        role: "system",
        content: `You are a helpful WhatsApp assistant. Respond naturally and helpfully to customer messages. Keep responses concise and friendly. Customer name: ${contact?.name || "Customer"}${trainingContext}`,
      },
    ];

    // 3. Add conversation history (excluding the current message to avoid duplication)
    // The current message is already passed as 'userMessage'.
    const history = conversationHistory
      .filter(msg => msg.content !== userMessage || (Date.now() - new Date(msg.createdAt).getTime() > 5000)) // Filter out "just saved" current message
      .slice(0, 10)
      .reverse();

    history.forEach((msg) => {
      const role = msg.fromUser || msg.fromType === 'bot' ? "assistant" : "user";
      messages.push({
        role,
        content: msg.content,
      });
    ...
    console.log(`[AI] Prepared ${messages.length} messages for ${provider} API`);
...
async function handleTemplateStatusUpdate(value: any, wabaId?: string) {
  ...
  console.log(
    `[Template Status] Update received: ${message_template_name} (WA ID: ${message_template_id}) - ${event}${reason ? ` - Reason: ${reason}` : ""
    } - WABA: ${wabaId || "unknown"}`
  );
  ...
  if (message_template_id && event) {
    ...
    if (template) {
      ...
      console.log(
        `[Template Status] Updated template "${template.name}" (channel: ${template.channelId}) status to ${status}${reason ? ` with reason: ${reason}` : ""
        }`
      );
    } else {
