...
export const getContactNotes = asyncHandler(async (req: Request, res: Response) => {
  const { id: contactId } = req.params;
  const notes = await storage.getContactNotes(contactId);
  res.json(notes);
});

export const createContactNote = asyncHandler(async (req: Request, res: Response) => {
  const { id: contactId } = req.params;
...
export const deleteContactNote = asyncHandler(async (req: Request, res: Response) => {
  const { noteId } = req.params;
  const success = await storage.deleteContactNote(noteId);
  if (!success) {
