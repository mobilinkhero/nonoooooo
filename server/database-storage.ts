...
export class DatabaseStorage implements IStorage {
  ...
  async getContactNotes(contactId: string): Promise<any[]> {
    const { contactNotes, users } = await import("@shared/schema");
  ...
  async deleteContactNote(id: string): Promise<boolean> {
    const { contactNotes } = await import("@shared/schema");
    const result = await db.delete(contactNotes).where(eq(contactNotes.id, id)).returning();
    return result.length > 0;
  }

  // Canned Responses
  async getCannedResponses(channelId: string): Promise<any[]> {
    const { cannedResponses } = await import("@shared/schema");
    const { eq, desc } = await import("drizzle-orm");
    return await db.select()
      .from(cannedResponses)
      .where(eq(cannedResponses.channelId, channelId))
  ...
  async deleteCannedResponse(id: string): Promise<boolean> {
    const { cannedResponses } = await import("@shared/schema");
    const { eq } = await import("drizzle-orm");
    const result = await db.delete(cannedResponses).where(eq(cannedResponses.id, id)).returning();
    return result.length > 0;
  }
