...
export class AutomationTriggerService {
  ...
  async handleMessageReceived(conversationId: string, message: any, channelId: string, contactId?: string, isNewConversation: boolean = false) {
    console.log(`💬 Message received trigger: ${conversationId}, isNewConversation: ${isNewConversation}`);
    ...
    console.log(`🎯 Found ${activeAutomations.length} active message_received automation(s) for channel ${channelId}`);
    ...
    for (const automation of activeAutomations) {
      // ✅ Check trigger config for frequency settings
      const config = (automation.triggerConfig as any) || {};
      const frequency = config.triggerFrequency || 'always'; // always, new_conversation_only

      console.log(`🤖 Checking automation "${automation.name}" (ID: ${automation.id}) with frequency: ${frequency}, isNew: ${isNewConversation}`);

      if (frequency === 'new_conversation_only' && !isNewConversation) {
        console.log(`⏭️ Skipping automation "${automation.name}" - set to trigger only on new conversations`);
        continue;
      }
