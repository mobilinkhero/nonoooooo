...
export class WhatsAppApiService {
  ...
  async markMessageAsRead(messageId: string): Promise<any> {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        console.warn(`[WhatsApp API] Failed to mark message as read: ${error.error?.message || "Unknown error"}`);
        return null; // Don't throw to avoid crashing the AI response flow
      }

      return await response.json();
    } catch (e) {
      console.warn(`[WhatsApp API] Exception marking message as read:`, e);
      return null;
    }
  }
