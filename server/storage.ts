...
export interface IStorage {
  ...
  // Contact Notes
  getContactNotes(contactId: string): Promise<any[]>;
  createContactNote(data: any): Promise<any>;
  deleteContactNote(id: string): Promise<boolean>;

  // Canned Responses
  getCannedResponses(channelId: string): Promise<any[]>;
  createCannedResponse(data: any): Promise<any>;
  updateCannedResponse(id: string, data: any): Promise<any>;
...
export class MemStorage implements IStorage {
  ...
  private contactNotes: Map<string, any> = new Map();
  private cannedResponsesMap: Map<string, any> = new Map();
  ...
  async createContactNote(data: any): Promise<any> {
    const id = randomUUID();
    const note = { ...data, id, createdAt: new Date(), updatedAt: new Date() };
    this.contactNotes.set(id, note);
    return note;
  }

  async deleteContactNote(id: string): Promise<boolean> {
    return this.contactNotes.delete(id);
  }

  // Canned Responses
  async getCannedResponses(channelId: string): Promise<any[]> {
    return Array.from(this.cannedResponsesMap.values())
      .filter(r => r.channelId === channelId)
      .sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
  }

  async createCannedResponse(data: any): Promise<any> {
    const id = randomUUID();
    const response = { ...data, id, createdAt: new Date(), updatedAt: new Date() };
    this.cannedResponsesMap.set(id, response);
    return response;
  }

  async updateCannedResponse(id: string, data: any): Promise<any> {
    const response = this.cannedResponsesMap.get(id);
    if (!response) throw new Error("Canned response not found");
    const updated = { ...response, ...data, updatedAt: new Date() };
    this.cannedResponsesMap.set(id, updated);
    return updated;
  }

  async deleteCannedResponse(id: string): Promise<boolean> {
    return this.cannedResponsesMap.delete(id);
  }
