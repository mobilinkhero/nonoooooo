...
const QuickRepliesDialog = ({
  ...
  const { data: replies = [], isLoading } = useQuery({
    queryKey: ["/api/canned-responses", channelId],
    queryFn: async () => {
      const response = await fetch(`/api/canned-responses?channelId=${channelId}`);
      if (!response.ok) throw new Error("Failed to load canned responses");
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => {
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <DialogTrigger asChild>
              <Button variant="ghost" size="icon" className="h-8 w-8 md:h-9 md:w-9 text-amber-500 hover:text-amber-600 hover:bg-amber-100/50 rounded-full transition-colors">
      ...
      <DialogContent className="max-w-xl p-0 overflow-hidden bg-white/95 backdrop-blur-xl border border-gray-200/60 shadow-2xl">
        <DialogHeader className="px-6 pt-6 pb-4 border-b border-gray-100 bg-gray-50/50">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 flex border rounded-xl items-center justify-center bg-white shadow-sm">
              <Zap className="h-5 w-5 text-amber-500" />
            </div>
            <div>
              <DialogTitle className="text-lg font-semibold text-gray-900">Quick Replies</DialogTitle>
              <DialogDescription className="text-sm text-gray-500 mt-0.5">Click a snippet below to instantly drop it into your message.</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="p-4 bg-gray-50/30">
          <div className="relative group">
            ...
            <Input
              placeholder="Search shortcuts (e.g., /pricing) or message content..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="pl-10 h-11 bg-white border-gray-200 focus:border-amber-400 focus:ring-amber-400/20 shadow-sm transition-all rounded-xl"
              autoFocus
            />
          </div>
        </div>

        <ScrollArea className="h-[360px] px-2 mb-2">
          <div className="px-2 space-y-2 pb-4">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center py-16 text-gray-400">
                <Loading />
                <span className="text-sm mt-3 animate-pulse">Loading swift replies...</span>
              </div>
              ...
              <div className="text-center py-12 px-4">
                ...
                <h3 className="text-sm font-semibold text-gray-900">No replies found</h3>
                <p className="text-sm text-gray-500 mt-1 mb-4">You don't have any quick replies matching your search.</p>
                <Button variant="outline" size="sm" onClick={() => (window.location.href = "/settings?tab=quick_replies")} className="rounded-full">
                ...
                <div
                  ...
                  onClick={() => {
                    ...
                    toast({
                      ...
                      description: "The shortcut text has been added to your chat box.",
                      duration: 2000,
                    });
                    setOpen(false);
                  }}
                  className="group relative bg-white border border-gray-200 rounded-xl p-4 cursor-pointer hover:border-amber-300 hover:shadow-[0_4px_20px_-4px_rgba(251,191,36,0.15)] transition-all duration-200 overflow-hidden"
                >
                  <div className="absolute inset-0 bg-gradient-to-r from-amber-50/0 via-amber-50/0 to-amber-50/50 opacity-0 group-hover:opacity-100 transition-opacity" />
                  <div className="relative flex justify-between items-start">
                    <div className="flex-1 pr-4">
                      <div className="flex items-center gap-2 mb-2">
                        <Badge variant="secondary" className="bg-gray-100 hover:bg-gray-200 text-gray-700 font-mono text-xs px-2 py-0.5 border-none">
                          /{reply.shortcut}
                        </Badge>
                      </div>
                      <p className="text-[13.5px] text-gray-600 leading-relaxed font-normal whitespace-pre-wrap line-clamp-3 group-hover:text-gray-900 transition-colors">
                        {reply.content}
                      </p>
                    </div>
                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gray-50 text-gray-400 group-hover:bg-amber-100 group-hover:text-amber-600 transition-colors flex-shrink-0 opacity-0 group-hover:opacity-100 transform translate-y-1 group-hover:translate-y-0">
...
const MessageComposer = ({
  ...
  // Auto-resize textarea when text is set programmatically (e.g., via Quick Replies)
  useEffect(() => {
            ...
            <>
              ...
              <QuickRepliesDialog
                ...
                onSelectReply={(content) => {
                  if (onSetMessageText) {
                    onSetMessageText(messageText ? `${messageText} ${content}` : content);
                    setTimeout(() => {
