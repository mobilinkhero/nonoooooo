...
import { ArrowLeft, Save, Loader2, Zap, Clock } from "lucide-react";
...
interface HeaderProps {
  ...
  triggerConfig: any;
  setTriggerConfig: (config: any) => void;
  automation: any;
  onClose: () => void;
  onSave: () => void;
  isSaving: boolean;
  ...
  return (
    <div className="bg-white border-b border-gray-200 px-4 py-2.5 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-2.5">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 bg-gray-50 rounded-lg px-2.5 py-1.5 border border-gray-200">
            ...
            <Select value={trigger} onValueChange={setTrigger}>
              <SelectTrigger className="h-5 text-[11px] w-[110px] border-none bg-transparent shadow-none p-0 focus:ring-0 font-medium text-gray-700">
          ...
          {trigger === 'message_received' && (
            <div className="flex items-center gap-1.5 bg-gray-50 rounded-lg px-2.5 py-1.5 border border-gray-200">
              ...
              <Select
                value={triggerConfig?.triggerFrequency || 'always'}
                onValueChange={(v) => setTriggerConfig({ ...triggerConfig, triggerFrequency: v })}
              >
                <SelectTrigger className="h-5 text-[11px] w-[120px] border-none bg-transparent shadow-none p-0 focus:ring-0 font-medium text-gray-700">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="always">Every message</SelectItem>
                  <SelectItem value="new_conversation_only">New conversation only</SelectItem>
                </SelectContent>
              </Select>
            </div>
          )}
        ...
        <Badge
          variant="outline"
          className={`text-[10px] h-5 px-2 font-medium rounded ${automation?.id
